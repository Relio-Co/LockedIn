...
function FriendsScreen({ navigation }) {
  ...
  const fetchFriendsAndInvites = async () => {
    ...
    if (userSnap.exists()) {
      ...
      const friendQueries = friendIds.map((friendId) =>
        getDoc(doc(db, "users", friendId))
      );
      const friendSnaps = await Promise.all(friendQueries);
      const friendsData = friendSnaps.map((snap) => ({
        id: snap.id,
        ...snap.data(),
      }));
  ...
  const fetchGroups = async () => {
    ...
    if (userSnap.exists()) {
      ...
      const groupsData = groupSnaps.map((snap) => ({
        id: snap.id,
        ...snap.data(),
      }));
  ...
  const sendFriendRequest = async () => {
    ...
    if (
      ...
      currentUserData.friends.includes(targetUserId)
    ) {
      setLoading(false);
